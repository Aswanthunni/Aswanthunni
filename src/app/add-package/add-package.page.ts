...
export class AddPackagePage implements OnInit {
  ...
  addPackage(data) {
    ...
    return this.db.storage.executeSql('INSERT INTO packagetable (name, details, fees, month, isactive) VALUES (?, ?, ?, ?, ?)', data2)
    .then(res => {
      if (res) {
        this.fetchPackage();
      }
  ...
  updatePackage(data) {
    ...
    return this.db.storage.executeSql(`UPDATE packagetable SET name = ?, details = ?, fees = ? , month = ? WHERE id = ${data.id}`, data2)
    .then(res => {
      if (res) {
        this.fetchPackage();
      }
